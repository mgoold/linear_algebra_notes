...
$$-\frac{1}{N}\left\[\sum_{(i,j):r_{ij}=1}y_{ij}log(\hat{y}\_{ij}) + (1-y_{ij})log(1 - \hat{y}ij)\right\]$$

where *N* is the number of ratings with $y_{ij}$ either 0 or 1.  here $r_{ij} = 1$ if there is a rating (0 or 1).  Here $r_{ij} = 1$ is there is a rating (0 or 1) of item j by user i and 0 otherwise.
...
def gradient(df, Y, emb_user, emb_movie):
    """Computes gradient of MSE loss for matrix factorization.

    Prediction for (user i, movie j): dot(emb_user[i], emb_movie[j])

    Returns:
      grad_user:  same shape as emb_user
      grad_movie: same shape as emb_movie
    """
    ### BEGIN SOLUTION

    u = df["userId"].to_numpy(dtype=np.int64)
    m = df["movieId"].to_numpy(dtype=np.int64)
    r = df["rating"].to_numpy(dtype=np.float64)

    # Predictions and errors for each observed rating
    pred = np.sum(emb_user[u] * emb_movie[m], axis=1)   # dot per row
    err = r - pred                                      # length N
    ...
    # Accumulate gradients over observed entries
    # MSE = (1/N) * sum (r - u·v)^2
    # d/d u_i: (-2/N) * sum_{(i,j)} err_{ij} * v_j
    # d/d v_j: (-2/N) * sum_{(i,j)} err_{ij} * u_i
    for t in range(N):
        i = u[t]
        j = m[t]
        e = err[t]
        grad_user[i]  += (-2.0 / N) * e * emb_movie[j]
        grad_movie[j] += (-2.0 / N) * e * emb_user[i]

    ### END SOLUTION
    return grad_user, grad_movie
,

fill in the below function so that it will implement gradient descent in a matrix factorization context:
...
The gradient descent model must include a momentum component.  Here is the modified gradient with momentum:
...
The above matrix factorization gradient descent function should also add a bias parameter ofr each user, and another bias parameter for each movie.  The parameter is analogous to thei ncerpt coefficient in linear regression.  For the parameter matrices U and V, the function should add a bias term $u^\*$ which is a vector of dimension $n_u$ and $v^\*$ which is a vector of dimension $n_v$.  The equation for this model enhancement is $\hat{y}_{ij} = u_i^{\*} + v_j^{\*} + u_i\cdot{}v\_j$ , where $u_i^{\*}$ and $v_j^{\*}$ are scalars and $u_i\cdot{}v_j = (UV^T)\_{ij}$ .
